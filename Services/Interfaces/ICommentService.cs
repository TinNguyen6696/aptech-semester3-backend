...
namespace TalentShowcase.Api.Services.Interfaces
{
    ...
    {
        Task<Result<CommentListDto>> GetVideoCommentsAsync(int videoId, int page, int pageSize, int? currentUserId);
        Task<Result<CommentDto>> AddVideoCommentAsync(int userId, int videoId, CreateCommentRequest request);
        Task<Result<CommentListDto>> GetCommunityPostCommentsAsync(int postId, int page, int pageSize, int? currentUserId);
